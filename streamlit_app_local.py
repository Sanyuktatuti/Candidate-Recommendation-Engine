    ...
    if candidates:
        ...
        if st.button("Analyze Candidates", type="primary"):
            with st.spinner("Generating embeddings and computing similarities..."):
                ...
                # Use unified embedding service with automatic hierarchy
                if embedding_service.active_service == "tfidf":
                    # For TF-IDF, need to fit on all texts together
                    progress_bar.progress(20)
                    job_embedding, candidate_embeddings = embedding_service.get_job_and_candidate_embeddings(
                        job_text, candidate_texts
                    )
                else:
                    # For API services (OpenAI, Cohere, HF), get embeddings separately
                ...
                similarities = compute_similarity_scores(job_embedding, candidate_embeddings, is_basic_mode=embedding_service.active_service == "tfidf")
